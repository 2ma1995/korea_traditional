...
async function main() {
  ...
  for (const day of KRX_HOLIDAYS.keys()) {
    const d = new Date(`${day}T12:00:00Z`);
    assert.equal(d.toISOString().slice(0, 10), day, `없는 날짜: ${day}`);
    assert.ok(d.getUTCDay() !== 0 && d.getUTCDay() !== 6, `주말은 적지 않는다: ${day}`);
  }
  assert.equal([...KRX_HOLIDAYS.keys()].filter(day => day.startsWith('2026')).length, 17); // 거래소 공고 17일
  ...
  // 화면에 쓰는 이름과 다음 거래일 — 추석 이틀 + 주말을 건너 월요일, 연말은 신정·주말을 건너 1/4
  assert.equal(marketHours(kst('2026-09-24T16:00:00')).closedFor, '추석');
  assert.equal(marketHours(kst('2026-09-24T16:00:00')).nextOpen, '9/28(월)');
  assert.equal(marketHours(kst('2026-09-26T16:00:00')).closedFor, '주말');
  assert.equal(marketHours(kst('2026-12-31T16:00:00')).nextOpen, '1/4(월)');
  assert.equal(marketHours(kst('2026-09-23T16:00:00')).closedFor, undefined);
  // 전날(수)·다음 월요일은 연다
