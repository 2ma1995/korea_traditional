...
export default async function EventPage() {
  ...
  const breads: BuilderBread[] = PRODUCTS.filter((p) => p.inStock).map((p) => ({ productNo: p.productNo, name: p.name, price: p.price, baseRate: p.line === plan.targetLine ? plan.rate : 0 }));
  // Pairing remains available even when the produce price API is unavailable.
  const seasonal = SEASONAL_INGREDIENTS.filter((i) => isInSeason(i, today.getMonth() + 1));
  ...
  return <main id="main-content" className="page-width inner-page">
    ...
    <div className="builder-layout" id="bread-builder">
      <BreadBuilder breads={breads} ingredients={seasonal} prices={priceMap} termName={term.ko} />
